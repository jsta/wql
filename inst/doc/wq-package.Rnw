...
% \VignetteDepends{wq, zoo, ggplot2, grid, reshape2}
% \VignetteKeyword{ts}
...
\noindent In general, the \textbf{ggplot2} package offers easy customization of almost all graphic output from \textbf{wq} by saving the output and adding additional \textbf{ggplot2} commands.
