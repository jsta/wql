...
\title{\texttt{wq:} Exploring water quality monitoring data\\(version 0.3-1)}
...
print(plot(sfb, vars = c('chl', 'dox', 'spm'), num.col = 2))
@
    \caption{Plotting specific variables of a \texttt{"WqData"} object, in this case chlorophyll-\emph{a}, dissolved oxygen, and suspended particulate matter.}
    \label{fig:strip}
  \end{center}
\end{figure}
...
Historical water quality data are often suitable for analyzing as monthly time series, which permits the use of many existing time series functions. \texttt{tsMake} is a function for \texttt{WqData} objects that creates monthly time series for all variables at a single site or for a single variable at all sites, when the option \texttt{type = "ts.mon"}. All replicates are first averaged and then the mean is found for the depth layers of interest. If no layers are specified, all depths will be used. The \texttt{layer} argument allows for flexibility in specifying depths, including negative depths used as codes for, say, ``near botton'' or ``entire water column''. The default time series plot is convenient for a quick look at the series (Figure~\ref{fig:6chlTs}):
...
@
    \caption{Chlorophyll-\emph{a} ($\mu$g L$^{-1}$ year$^{-1}$) trends in San Francisco Bay.}
    \label{fig:mk}
  \end{center}
\end{figure}
...
\begin{figure}[tbp]
  \begin{center}
<<fig=TRUE, echo=FALSE, width=6.5, height=7>>=
...
<<>>=
plotSeason(chl27, num = 3, same.plot = FALSE, ylab = 'Stn 27 Chl-a')
@
...
\begin{figure}[tbp]
  \begin{center}
<<fig=TRUE, echo=FALSE, width=6, height=4>>=
print(plotSeason(chl27, num = 3, same.plot = FALSE, ylab = 'Stn 27 Chl-a'))
@
...
<<>>=
plotSeason(chl27, num = 3, same.plot = TRUE, ylab = 'Stn 27 Chl-a')
@

\begin{sidewaysfigure}[tbp]
  \begin{center}
<<fig=TRUE, echo=FALSE, width=8, height=4>>=
print(plotSeason(chl27, num = 3, same.plot = TRUE, ylab = 'Stn 27 Chl-a'))
@
...
\noindent \texttt{plotSeason} also has an option to plot all individual months separately for the entire record (Figure~\ref{fig:seas3}). 

<<>>=
plotSeason(chl27, "by.month", ylab = 'Stn 27 Chl-a', scales = "free_y")
@
...
\begin{figure}[tbp]
  \begin{center}
<<fig=TRUE, echo=FALSE>>=
print(plotSeason(chl27, "by.month", ylab = 'Stn 27 Chl-a', scales = "free_y"))
@
    \caption{Changing seasonal pattern in \texttt{chl27} described by the time series for individual months.}
    \label{fig:seas3}
  \end{center}
\end{figure}
\setkeys{Gin}{width=.8\textwidth}
