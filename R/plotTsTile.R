...
function(x, title = NULL, trim = TRUE, four = TRUE, loganom = TRUE) {
	...
	## four: If TRUE, divide into 4 special groups; otherwise deciles. Useful for log-anomaly data.
	...
	mypalette <- colorRampPalette(c('darkblue', 'lightblue', 'pink', 'red')) 
	...
	if(four){
		...
		if(mmin >= 0 | mmax <= 0) stop('Data all of one sign; you may want four = FALSE or loganom = TRUE.')
		x2 <- cut(x1, breaks = c(mmin, mlo, 0, mhi, mmax), labels = FALSE, include.lowest = TRUE)
		cols <- mypalette(4)
		key <- list(col = cols, at = seq(.5, 4.5, 1), labels = list(labels = signif(c(mmin, mlo, 0, mhi, mmax), 2), at = seq(.5, 4.5, 1)))
	...
	## calculate key if deciles	
		...
		cols <- mypalette(10)
		key <- list(col = cols, at = seq(.5, 10.5, 1), labels = list(labels = signif(decs, 2)[seq(1, 11, 2)], at = seq(.5, 10.5, 1)[seq(1, 11, 2)]))
	}
	
	## create data.frame for graph and plot it
	x3 <- data.frame(yr = floor(time(x1)), mon = factor(month.abb[cycle(x1)], levels = month.abb), value = x2)
	levelplot(value~yr+mon, data = x3, aspect = 'iso', 
		...
		scales = list(y = list(at = seq(1, 11, 2), labels = month.abb[seq(1, 11, 2)])), 
		xlab = '', ylab = '', 
		col.regions = cols, 
		colorkey = key
	)	
