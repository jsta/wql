...
### loganom: Should x be converted to log-anomaly data?
	...
	if(four){
		...
		the.breaks <- quantile(x1, probs = seq(0, 1, .1), na.rm = TRUE)
		labl <- seq(1, 11, 2)
	}
	len <- length(the.breaks)
   if(length(unique(the.breaks)) < len) stop("Breaks between  groups are
   	not unique: insufficient unique data.")
   x2 <- cut(x1, breaks = the.breaks, labels = FALSE, include.lowest
      = TRUE)
   cols <- mypalette(len - 1)
   the.seq = seq(.5, len - 0.5, 1)
   key <- list(col = cols, at = the.seq, labels = list(labels
      = signif(the.breaks, 2)[labl], at = the.seq[labl]))
	...
	levelplot(value~yr+mon, data = x3, aspect = 'iso', at=the.seq,
		main = title, 
		ylim = c(.5, 12.5), 
		panel = function(z, ...) {
			panel.levelplot(z, ...)
		...
		scales = list(y = list(at = seq(1, 11, 2), labels =
			month.abb[seq(1, 11, 2)])),
		xlab = '', ylab = '', 
		col.regions = cols, 
		colorkey = key
	)	
