plotSeasonTrend<- function(x, first, last,
	type = c("slope", "slope.pct"), method = c("mk", "lin"), plot = TRUE,
	miss = FALSE, leg = FALSE, ...)
	{

### Plot trends by season
### Args: 
###     x, ts or mts
###     first and last, first and last year
...
###     tsname, name of time series
    ...
    if (missing(first)) first = start(x)[1]
    if (missing(last)) last = end(x)[1]
    if (first < start(x)[1]) first <- start(x)[1]
    if (last > end(x)[1]) last <- end(x)[1]
    type <- match.arg(type)
    ...
    if (method == 'mk') {
        ...
        trend <-  function(x) {
            ...
            miss <- (fifth^2 - sum(!is.na(xbeg)) * sum(!is.na(xend))) /
            	fifth^2
    ...
    ts.id <- factor(rep(tsnames, each = fr), levels = tsnames, ordered =
    	TRUE)
    ans1 <- data.frame(trend = ans[, type], ans[, 3:4], season =
    	rep(1:fr, times = nc), ts.id)
