...
{
# aj 10/20/09 4:18 PM
# Find and rotate first n EOFs of a matrix, mts or data.frame. NAs not allowed.
# Args:
...
#   eigenvalue.pc: all eigenvalues of correlation matrix as percent of total variance
...
# return results
  ...
  reof <- cbind(id=ordered(colnames(x), levels=colnames(x)), as.data.frame(reof))
  amp <- cbind(id=ordered(rownames(x), levels=rownames(x)), as.data.frame(amp))	
  rownames(reof) <- rownames(amp) <- NULL
  list(REOF=reof, amplitude=amp, eigen.pct=eigen.pct, variance=totvar.pct)
}
